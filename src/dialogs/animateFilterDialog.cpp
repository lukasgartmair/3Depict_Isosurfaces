...
#include "viscontrol.h" //For tree update routine
enum
{
	ID_FILTER_TREE_CTRL,
	ID_PROPERTY_GRID,
    ...
    wxDialog(parent, id, title, pos, size, wxDEFAULT_DIALOG_STYLE|wxRESIZE_BORDER|wxMAXIMIZE_BOX|wxMINIMIZE_BOX)
{
    ...
    comboRangeFormat = new wxComboBox(frameViewPane, ID_COMBO_RANGE_TYPE, wxT(""), wxDefaultPosition, wxDefaultSize, rangeNames, wxCB_DROPDOWN|wxCB_READONLY|SAFE_CB_SORT);
...
void ExportAnimationDialog::updateFrameViewGrid()
{
	...
	for(size_t ui=0; ui<alteredProperties.size();ui++)
	{
		...
		//Moidify the grid properties with the appropriate data
...
void ExportAnimationDialog::OnFilterGridCellEditorShow(wxGridEvent &event)
{
	...
	switch(filterProp.type)
	{
		case PROPERTY_TYPE_BOOL:
		{
			...
			//Find the last property for the fitler from the animator
			// if available
			std::string sData;
...
void ExportAnimationDialog::OnButtonKeyFrameRemove(wxCommandEvent &event)
{
	...
	;wxGridCellCoordsArray selectedRows = animationGrid->GetSelectedCells();
	...
	{
		size_t rowToKill;
		rowToKill=animationGrid->GetGridCursorRow();
		propertyAnimator.removeNthKeyFrame(rowToKill);
	}
