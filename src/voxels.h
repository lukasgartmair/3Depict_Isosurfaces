...
template<class T> class Voxels
{
	...
	public:
		...
		Point3D getEdgeCentre(unsigned int edgeIndex,size_t &axis) const;
...
size_t Voxels<T>::getEdgeIndex(size_t x,size_t y, size_t z, unsigned int index) const
{
	...
	return y + z*(binCount[0]+1) + x*(binCount[0]+1)*(binCount[1]+1) +
			axis*(binCount[0]+1)*(binCount[1]+1)*(binCount[2]+1);

}

template<class T>
Point3D Voxels<T>::getEdgeCentre(unsigned int edgeIndex, size_t &axis) const
{
	//Invert the mapping generated by the edgeIndex function
	//to retrieve the XYZ and axis values
	size_t x,y,z;
	
	axis=edgeIndex/((binCount[0]+1)*(binCount[1]+1)*(binCount[2]+1));
	edgeIndex-=axis*(binCount[0]+1)*(binCount[1]+1)*(binCount[2]+1);

	ASSERT(axis < 3);

	//The XY and Z offsets can extend slightly  (by one cell) beyond the dataset
	x= edgeIndex/((binCount[0]+1)*(binCount[1]+1));
	edgeIndex-=x*(binCount[0]+1)*(binCount[1]+1);

	ASSERT(x <= binCount[0]);

	y=edgeIndex/(binCount[0]+1);
	edgeIndex-=y*(binCount[0]+1);
	...
	Point3D cellCentre;
	cellCentre = Point3D((float)x/(float)binCount[0]*(maxBound[0]-minBound[0]) + minBound[0],
			(float)y/(float)binCount[1]*(maxBound[1]-minBound[1]) + minBound[1],
			(float)z/(float)binCount[2]*(maxBound[2]-minBound[2]) + minBound[2]);

	
	Point3D delta= maxBound-minBound;
	delta[0]/=binCount[0];
	delta[1]/=binCount[1];
	delta[2]/=binCount[2];
	switch(axis)
	{
		case 0:
			return cellCentre - Point3D(delta[0],0,0);
		case 1:
			return cellCentre - Point3D(0,delta[1],0);
		case 2:
			return cellCentre - Point3D(0,0,delta[2]);
	}
